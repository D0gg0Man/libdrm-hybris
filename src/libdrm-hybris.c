...
int
drmModeCreateLease(int fd, const uint32_t *o, int n, int f, uint32_t *id)
{
    if (!is_compositor()) {
...
{
    if (!real_poll)
        real_poll = (int (*)(struct pollfd *, nfds_t, int))
            resolve_next("poll", (void *)poll);
...
copy_to_dumb(buffer_handle_t h)
{
    if (!dumb_map || !h || !frame_w || !frame_h)
        return;
    void *src = NULL;
    if (hybris_gralloc_lock(h, 0x3 | 0x30, 0, 0, frame_w, frame_h, &src) || !src)
        return;
...
int
drmModeAddFB2WithModifiers(int fd, uint32_t w, uint32_t h, uint32_t fmt,
...
int
drmModeAddFB2(int fd, uint32_t w, uint32_t h, uint32_t fmt,
    const uint32_t handles[4], const uint32_t pitches[4], const uint32_t offsets[4],
...
int
drmModeSetCrtc(int fd, uint32_t crtcId, uint32_t bufferId, uint32_t x, uint32_t y,
...
int
drmModePageFlip(int fd, uint32_t crtc_id, uint32_t fb_id, uint32_t flags, void *ud)
{
